...
- `-с`/`JSON_CONFIG` - путь до json-файла с конфигом приложения <br>
- `g` /`GRPC_PORT` - порт для gRPC-сервера - полный адрес сервера - комбинация значений `SERVER_ADDRESS` и `GRPC_PORT` 
...
## Сертификаты для локального запуска HTTPS-сервера
Для локального запуска HTTPS-сервера необходимо сгенерировать сертификат и публичный ключ при помощи утилиты mkcert. 

Команды для установки утилиты:
- MacOS - `brew install mkcert` и `brew install nss`
- Linux - `sudo apt update`, `sudo apt install libnss3-tools`, `sudo apt install mkcert`

Инициализируем локальный Certificate Authority:
- `mkcert -install` 

Команда для генерации сертификата и приватного ключа:
- `mkcert localhost 127.0.0.1`

Данная команда создаст файлы `127.0.0.1+1.pem` (сертификатов) и `127.0.0.1+1-key.pem`(публичный ключ) в корневой директории проекта. 
